  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="glassmorphism rounded-2xl p-6 md:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-[30%] h-full bg-emerald-500/5 blur-[50px] pointer-events-none" />
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-800/80 border border-slate-700/50 text-xs text-slate-300 font-medium z-10 shadow-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div className="glassmorphism rounded-xl p-5 border border-slate-800/80 flex flex-col gap-4 relative">
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-xs text-slate-500">/ {totalVehicles} in fleet</span>
          </div>
          <div className="text-[11px] text-slate-400 flex items-center gap-1 border-t border-slate-800/50 pt-3">
        ...
        {/* Card 2: Active Trips */}
        <div className="glassmorphism rounded-xl p-5 border border-slate-800/80 flex flex-col gap-4">
          <div className="flex items-center justify-between">
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-xs text-slate-500">trips in transit</span>
          </div>
          <div className="text-[11px] text-slate-400 flex items-center gap-1 border-t border-slate-800/50 pt-3">
        ...
        <div className="glassmorphism rounded-xl p-5 border border-slate-800/80 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Available Drivers</span>
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-xs text-slate-500">drivers on stand-by</span>
          </div>
          <div className="text-[11px] text-slate-400 flex items-center gap-1 border-t border-slate-800/50 pt-3">
        ...
        <div className="glassmorphism rounded-xl p-5 border border-slate-800/80 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Operating Costs</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glassmorphism rounded-xl p-6 border border-slate-800/80 flex flex-col gap-4">
          <h3 className="text-sm font-bold text-white uppercase tracking-wider">Active Trip Logs</h3>
          <div className="flex flex-col gap-3">
            {trips.length === 0 ? (
              <p className="text-xs text-slate-500 py-6 text-center">No trips currently logged in system.</p>
            ) : (
              trips.map((trip) => (
                <div key={trip.id} className="p-4 rounded-lg bg-slate-900/60 border border-slate-800 flex items-center justify-between gap-4">
                  <div className="flex flex-col gap-1 min-w-0">
                    <span className="text-xs font-mono font-bold text-slate-400">{trip.tripNumber}</span>
                    <span className="text-sm text-slate-200 truncate">{trip.origin} → {trip.destination}</span>
                    <span className="text-[11px] text-slate-500 truncate">Cargo: {trip.cargoDescription} ({trip.cargoWeightKg} kg)</span>
                  </div>
                  <div className="flex flex-col items-end gap-1.5 shrink-0">
                    <span className={`px-2 py-0.5 text-[10px] font-semibold tracking-wider uppercase rounded-full border
                      ${trip.status === 'on_trip' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20 animate-pulse-soft' : ''}
                      ${trip.status === 'scheduled' ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' : ''}
                      ${trip.status === 'completed' ? 'bg-slate-500/10 text-slate-400 border-slate-500/20' : ''}
                      ${trip.status === 'cancelled' ? 'bg-red-500/10 text-red-400 border-red-500/20' : ''}
                    `}>
                      {trip.status.replace('_', ' ')}
                    </span>
                    <span className="text-[10px] text-slate-500">Est. {trip.estimatedDistanceKm} km</span>
                  </div>
                </div>
              ))
            )}
        ...
        <div className="glassmorphism rounded-xl p-6 border border-slate-800/80 flex flex-col gap-4">
          <h3 className="text-sm font-bold text-white uppercase tracking-wider">Alerts & Compliance</h3>
          <div className="flex flex-col gap-3">
            {criticalDrivers.length === 0 ? (
              <div className="p-4 rounded-lg bg-slate-900/40 border border-slate-800 flex flex-col items-center justify-center py-8 text-center">
                <ShieldCheck size={28} className="text-emerald-500 mb-2" />
                <p className="text-xs font-semibold text-slate-200">System Fully Compliant</p>
                <p className="text-[10px] text-slate-500 mt-1">All driver licenses & vehicle registrations healthy.</p>
              </div>
            ) : (
                ...
                return (
                  <div key={driver.id} className="p-4 rounded-lg bg-red-500/5 border border-red-500/20 flex gap-3">
                    ...
                    <div className="flex flex-col gap-0.5">
                      <span className="text-xs font-bold text-slate-200">License Expiring: {driver.fullName}</span>
                      <span className="text-[11px] text-slate-400">License: {driver.licenseNumber} ({driver.licenseClass})</span>
